...
def get_or_create_page(s: requests.Session, parent_id: str, space_key: str,
    ...
    """Return page_id: create new or update existing page with same title."""
    ...
    if existing:
        ...
        # Update with placeholder so attachments can be re-uploaded
...
def data_path_macro(session_id: str) -> str:
    return (
        ...
        f'<ac:rich-text-body>Session : {session_id}\n'
        f'Data    : ~/git/cgreclaim/tools/data/{session_id}/\n'
        f'Report  : ~/git/cgreclaim/tools/report/{session_id}/</ac:rich-text-body>'
        f'</ac:structured-macro>'
...
def main():
    ...
    # Step 1: create child pages first (need IDs for attachment + links)
    child_pages: dict = {}  # title -> page_id

    # Syslog child page
    if has_syslog:
        print("\n[1/3] Creating Syslog 타이밍 분석 page...")
        # create with placeholder, then upload PNGs, then update
        syslog_title = f"Syslog 타이밍 분석 [{session_id}]"
        syslog_id = get_or_create_page(s, args.parent_page_id, space_key,
                                       syslog_title, "")
        if has_timeline:
    ...
    if has_mem_data:
        ...
        mem_id = get_or_create_page(s, args.parent_page_id, space_key,
                                    mem_title, "")
        if has_meminfo:
    ...
    if has_cg_data:
        ...
        cg_id = get_or_create_page(s, args.parent_page_id, space_key,
                                   cg_title, "")
        if has_cgroup:
    ...
    # Step 2: create or update parent summary page
    ...
    # Check if page with this title already exists under the parent
    r = s.get(f"{CONFLUENCE_BASE}/rest/api/content",
    ...
    if existing:
        ...
        update_page_content(s, parent_id, summary_body)
    else:
        parent_id = create_child_page(s, args.parent_page_id, space_key,
                                      parent_title, summary_body)

    print(f"\n✅ Done!")
    print(f"  Parent : {CONFLUENCE_BASE}/pages/viewpage.action?pageId={parent_id}")
    for title, pid in child_pages.items():
